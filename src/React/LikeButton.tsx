...
const LikeButton = () => {
  ...
  const borderColorClass = isLiked
    ? "border-[var(--sec)]"
    : "border-[var(--black-icon)]";

  const svgClasses = `
    w-6 h-6 transition-all duration-300 ease-in-out 
    ${isLiked ? "text-[var(--sec)] scale-110" : "text-[var(--black-icon)] group-hover:text-[var(--black)] group-hover:scale-105"}
    ...
    <div className="flex items-center">
      <button
        ...
        className={`
          ...
          ${!isLiked ? "md:hover:border-[var(--black)]" : ""}
          ${triggerAnimation ? " animate-scale" : ""}
        `}
      >
        <svg
